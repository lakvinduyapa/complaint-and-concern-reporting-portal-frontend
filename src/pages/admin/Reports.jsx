...
const Reports = () => {
  ...
  const filterComplaintsByDate = (complaints) => {
    ...
    if (isInvalidDateRange(filterFrom, filterTo)) return [];
  ...
  const viewPDF = async () => {
    ...
    if (isInvalidDateRange(filterFrom, filterTo)) {
  ...
  const handleFilterFromChange = (e) => {
    ...
    if (filterTo && newFrom && new Date(newFrom) > new Date(filterTo)) {
      alert("Start Date cannot be after End Date");
      return;
    }
  ...
  const handleFilterToChange = (e) => {
    ...
    if (filterFrom && newTo && new Date(filterFrom) > new Date(newTo)) {
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-6 md:p-8 mb-6">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-2">
            Monitor complaint trends, investigation progress, and operational
            statistics.
          </p>
        </div>
