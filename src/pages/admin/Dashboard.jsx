...
const Dashboard = () => {
  ...
  // 1. Memoized Chart & Statistic Computations to prevent lag on layout re-renders
  ...
  const anonymousCount = useMemo(() => {
    return allComplaints.filter((c) => c.is_anonymous).length;
  }, [allComplaints]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-6">
        {primaryStats.map((card, index) => (
          <div
            key={index}
            className={`${card.bgColor} relative overflow-hidden h-[180px] p-6 rounded-3xl border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 cursor-pointer animate-fade-up`}
      ...
      <div>
        <div className="mb-6">
          ...
          <p className="text-slate-500 mt-1">Complaint trends and distribution</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-3xl shadow-lg border border-slate-200 p-6 animate-fade-up">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-slate-500 text-sm">Anonymous Complaints</p>
                <h3 className="text-4xl font-bold text-slate-900">{anonymousCount}</h3>
                <p className="text-sm text-slate-500">
                  {allComplaints.length > 0 ? Math.round((anonymousCount / allComplaints.length) * 100) : 0}% of complaints
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-slate-200 p-6 animate-fade-up">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-slate-500 text-sm">Named Complaints</p>
                <h3 className="text-4xl font-bold text-slate-900">{namedCount}</h3>
                <p className="text-sm text-slate-500">
                  {allComplaints.length > 0 ? Math.round((namedCount / allComplaints.length) * 100) : 0}% of complaints
                </p>
              </div>
            </div>
          </div>
      ...
      {!chartsLoading && allComplaints.length > 0 && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-6 animate-fade-up">
            ...
            {/* Added debounce={150} and changed width to 99% to avoid flex loops */}
            <ResponsiveContainer width="99%" height={isMobile ? 320 : 350} debounce={150}>
              <PieChart>
                <Pie
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-6 animate-fade-up">
            ...
            {/* Added debounce={150} and changed width to 99% to avoid flex loops */}
            <ResponsiveContainer width="99%" height={isMobile ? 340 : 350} debounce={150}>
              <BarChart data={chartCategoryData} margin={{ top: 10, right: 10, left: -18, bottom: 42 }}>
                ...
                <defs>
                  <linearGradient id="categoryGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#1456C4" />
                    <stop offset="100%" stopColor="#005E5E" />
                  </linearGradient>
                </defs>
                <Bar dataKey="value" fill="url(#categoryGradient)" radius={[12, 12, 0, 0]} animationDuration={1800} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
